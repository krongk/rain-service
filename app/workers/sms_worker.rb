class SmsWorker
  ...
  def perform(current_user_id, sms_tmp_id, phone_item_ids)
    ...
    PhoneItem.where(:id => phone_item_ids).each do |phone_item|
	    ...
	    #update flag
	    phone_item.is_processed = item.is_processed == 'n' ? "#{sms_tmp.id},#{status_id}" : "#{sms_tmp.id},#{status_id}|" + phone_item.is_processed
	    phone_item.save!
	    #write log
	    ...
	    SmsLog.create!(:user_id => current_user_id, 
	    		:phone_item_id => phone_item.id, 
	    		:sms_tmp_id => sms_tmp.id, 
	    		:status => status_id,
	    		:billing_count => billing_count)
	    #increment billing count
	    Keystore.increment_value_for("user:#{item.user_id}:sms_billing_count", billing_count)
	    #increment send count(per phone per send)
	    Keystore.increment_value_for("user:#{item.user_id}:sms_send_count", 1)
	    puts billing_count
	end
